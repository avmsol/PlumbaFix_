...
import { ArrowLeft, Filter, Search, CheckCircle2, Clock, Wrench, User, MapPin, DollarSign, Calendar, Eye, Navigation, Gift, Star, AlertCircle } from 'lucide-react';
...
export default function Jobs({ onNavigate, jobs, onViewJob }: JobsProps) {
  ...
  const activeJobs = filteredJobs.filter(j => j.status !== 'completed' && j.status !== 'pending-approval');
  const completedJobs = filteredJobs.filter(j => j.status === 'completed');
  const pendingApprovalJobs = filteredJobs.filter(j => j.status === 'pending-approval');
  ...
  const getStatusBadge = (status: JobData['status']) => {
    const statusConfig = {
      ...
      'pending-approval': { label: 'Awaiting Approval', color: 'bg-amber-100 text-amber-700' },
    };
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex gap-2">
                {job.status === 'pending-approval' && (
                  <Button 
                    size="sm" 
                    className="bg-[#00C853] hover:bg-green-700 gap-1 rounded-lg"
                    onClick={() => onNavigate('job-approval')}
                  >
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    Approve
                  </Button>
                )}
                {job.status !== 'completed' && job.status !== 'pending-approval' && job.jobType === 'plumber' && job.status !== 'pending' && (
                  <Button 
                    size="sm" 
                    variant="outline"
                ...
                >
                  ...
                  {job.status === 'pending-approval' ? 'View Details' : 
  ...
  return (
    <div className="h-full flex flex-col bg-white relative">
      {/* Header */}
      <div className="px-4 lg:px-6 pt-14 pb-4 lg:pt-6 lg:pb-4 border-b border-gray-100 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-4 mb-2">
            ...
            <div className="flex-1">
              <h1 className="text-xl lg:text-2xl mb-0.5">My Jobs</h1>
              <p className="text-gray-500 text-xs">Track all your repairs and services</p>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto p-6 lg:p-8">
          <Tabs defaultValue="all" className="w-full">
            <TabsList className="grid w-full grid-cols-4 mb-6 bg-gray-100 rounded-xl p-1">
              <TabsTrigger value="all" className="rounded-lg text-xs lg:text-sm">
                All ({filteredJobs.length})
              </TabsTrigger>
              <TabsTrigger value="active" className="rounded-lg text-xs lg:text-sm">
                Active ({activeJobs.length})
              </TabsTrigger>
              <TabsTrigger value="pending-approval" className="rounded-lg text-xs lg:text-sm relative">
                Approval {pendingApprovalJobs.length > 0 && (
                  <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white rounded-full text-xs flex items-center justify-center">
                    {pendingApprovalJobs.length}
                  </span>
                )}
              </TabsTrigger>
              <TabsTrigger value="completed" className="rounded-lg text-xs lg:text-sm">
            ...
            <TabsContent value="pending-approval" className="space-y-3 mt-0">
                ...
                <>
                  <Card className="p-4 border-amber-200 bg-amber-50">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h3 className="text-amber-900 mb-1">Action Required</h3>
                        <p className="text-sm text-amber-700">
                          Please review and approve completed work to release payment to your plumber.
                        </p>
                      </div>
                    </div>
                  </Card>
                  {pendingApprovalJobs.map(job => <JobCard key={job.id} job={job} />)}
                </>
              ) : (
                <EmptyState message="No jobs pending approval." />
              )}
            </TabsContent>
